...
## A
### Annotation ($Entity)
A JSON format representing the objects and labels exists in an $Item.
### Artifacts ($Entity)
...
### Box ($Entity)
A bounding box $Annotation type. Represented by 2 point for top-left and bottom-right. type=`box`
## C
### Clone
...
### Polygon ($Entity)
A polygon $Annotation type. Represented by a list of (x,y) points. type=`segment`
## Q
## R
### Repository
...
## S
### Segmentation ($Entity)
A binary mask $Annotation type. Represented by mask with the same size as the image, 0 for background, 1 for the object. type=`binary`
### Service ($Entity)
A deployed $Package that serves the code. Given the matching input to a function, it will run it and return the output, e.g. if we have code in our $Package for converting RGB images to grayscale, the dl.Service would run the code and upload the grayed image.
## T
### Task ($Entity)
