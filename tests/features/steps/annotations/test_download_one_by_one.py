...
def step_impl(context):
    try:
        ...
        context.item_3 = context.dataset.items.upload(local_path=context.items_filepath[2],
                                                      remote_path=context.new_path)
...
def step_impl(context):
    ...
    try:
        ...
        assert "Failed to find {}\n{}".format(os.path.join(context.root_path, 'apple_77.json'), e)
    ...
    try:
        ...
        assert "Failed to find {}\n{}".format(os.path.join(context.root_path, 'folder_a/apple_78.json'), e)

    assert len(context.item_2.annotations.list()) == len(context.annotations), "Missing annotations in file apple_78.json"

    try:
        with open(os.path.join(context.root_path, context.new_path, 'apple_79.json'), "r") as f:
            context.annotations = json.load(f)["annotations"]
    except Exception as e:
        assert "Failed to find {}\n{}".format(os.path.join(context.root_path, context.new_path, 'apple_79.json'), e)
